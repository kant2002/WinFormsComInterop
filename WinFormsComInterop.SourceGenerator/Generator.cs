...
";
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            {
                ProcessClassDeclaration(classSymbol, context);
            }
        }
        private string ProcessClassDeclaration(ClassDeclaration classSymbol, INamedTypeSymbol interfaceTypeSymbol, string aliasSymbol)
        {
...
");
            ...
            source.AppendLine($"partial unsafe class {interfaceTypeSymbol.Name}Proxy");
            source.AppendLine("{");
            source.PushIndent();
                    ...
                    case IMethodSymbol methodSymbol:
                        {
                            ...
                            GenerateMethod(source, interfaceTypeSymbol, aliasSymbol, methodContext);
                        }
                        break;
        ...
        private void ProcessClassDeclaration(ClassDeclaration classSymbol, GeneratorExecutionContext context)
        {
            ...
            foreach (var proxyAttribute in proxyDeclarations)
            {
                ...
                var aliasSymbol = proxyAttribute.ConstructorArguments.ElementAtOrDefault(1).Value as string;
                var sourceCode = ProcessClassDeclaration(classSymbol, interfaceTypeSymbol, aliasSymbol);
                var key = (INamedTypeSymbol)classSymbol.Type;
                var typesuffix = interfaceTypeSymbol.FormatType(aliasSymbol).Replace(".", "_").Replace("::", "_");
                context.AddSource($"{key.ToDisplayString().Replace(".", "_")}_{typesuffix}_comcallablewrapper.cs", SourceText.From(sourceCode, Encoding.UTF8));
            }
        }

        private Marshaller CreateMarshaller(IParameterSymbol parameterSymbol, string typeAlias)
        {
            ...
            marshaller.TypeAlias = typeAlias;
            return marshaller;
        }
        ...
        private void GenerateMethod(IndentedStringBuilder source, INamedTypeSymbol interfaceSymbol, string alias, MethodGenerationContext context)
        {
            ...
            {
                var marshaller = CreateMarshaller(_, alias);
                return marshaller;
            });
            ...
            source.AppendLine($"var inst = ComInterfaceDispatch.GetInstance<{interfaceSymbol.FormatType(alias)}>((ComInterfaceDispatch*)thisPtr);");
